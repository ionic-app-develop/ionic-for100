var modalSelectTemplates = modalSelectTemplates || {};modalSelectTemplates['modal-template-multiple.html'] = ' <ion-modal-view class="ionic-select-modal" ng-class="::ui.modalClass">\n' +
    ...
    '      <h1 class="title text-center">{{::ui.modalTitle}} MULTIPLE</h1>\n' +
    '    </ion-header-bar>\n' +
    '    <ion-content>\n' +
    '\n' +
    ...
    '        <button class="button button-positive" ng-click="closeModal()">{{ui.cancelButton}}</button>\n' +
    '        <h2 class="title"><button class="button">OK</button></h2>\n' +
    '        <button ng-if="::!ui.hideReset" class="button button-positive" ng-click="unsetValue()">{{ui.resetButton}}</button>\n' +
    '    </ion-footer-bar>\n' +
    '</ion-modal-view>\n' +
    '';
...
var modalSelectTemplates = modalSelectTemplates || {};modalSelectTemplates['modal-template.html'] = ' <ion-modal-view class="ionic-select-modal" ng-class="::ui.modalClass">\n' +
    ...
    '    </div>\n' +
    '\n' +
    '    <ion-content class="has-header" style="text-align:center;" ng-class="{\'has-subheader\':ui.hasSearch}">\n' +
    '        <div class="text-center" style="font-size:14px;" ng-if="!ui.shortList && !showList" style="padding-top:40px;">\n' +
    '            <h4 class="muted">{{::ui.loadListMessage}}</h4>\n' +
    '            <p>\n' +
    '                <ion-spinner></ion-spinner>\n' +
    '            </p>\n' +
    ...
    '            <div ng-if="!ui.shortList">\n' +
    '                <div class="list" ng-if="showList" class="animate-if">\n' +
    '                    <div class="item item-text-wrap" style="font-size:14px;" collection-repeat="option in options track by $index" ng-click="setOption(option)" ng-class="{\'{{::ui.selectedClass}}\': compareValues(getSelectedValue(option), ui.value) }"> \n' +
    '                        <div compile="inner" compile-once="true"></div>\n' +
    '                    </div>\n' +
    '                </div>\n' +
    '            </div>\n' +
    '            <div ng-if="ui.shortList">\n' +
    '                <div class="list">\n' +
    '                    <div class="item item-text-wrap" style="font-size:14px;" ng-repeat="option in options track by $index" ng-click="setOption(option)" ng-class="{\'{{::ui.selectedClass}}\': compareValues(getSelectedValue(option), ui.value) }">\n' +
    '                        <div compile="inner" compile-once="true"></div>\n' +
    '                    </div>\n' +
    '                </div>\n' +
    '            </div>\n' +
    ...
    '\n' +
    '    <ion-footer-bar>\n' +
    '    <div class="button-bar footer" style="padding: 0px 30px;">' +
    '        <button type="button" class="button button-positive" ng-click="closeModal()">{{ui.cancelButton}}</button>\n' +
    '    </div>' +
    '    </ion-footer-bar>\n' +
    '\n' +
    '</ion-modal-view>\n' +
    ...
    return {
        ...
        link: function (scope, iElement, iAttrs, ngModelController, transclude) {
            ...
            scope.ui = {
                ...
                headerFooterClass : iAttrs.headerFooterClass || 'bar-positive',
                value  : null,
                ...
                subHeaderClass : iAttrs.subHeaderClass || 'bar-positive',
